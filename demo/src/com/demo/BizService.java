...
public class BizService {
    ...
    /**
     * 模拟一段确定性的中等耗时业务处理（不涉及休眠）。
     *
     * <p>基准测试需要"毫秒级业务方法"来观察探针开销在真实耗时下的占比，
     * 但用 {@code Thread.sleep} 实现会引入操作系统定时器精度带来的巨大抖动
     * （Windows 默认约 15ms 粒度，实测 1ms 休眠实际耗时约 9ms 且波动明显），
     * 使测量噪声大于待测差异。改用确定性的算术循环后，
     * 单次耗时稳定在亚毫秒级，测量可重复。
     *
     * @param n 循环次数，约 20 万次对应 0.2~0.5 毫秒
     */
    public long simulatedWork(int n) {
