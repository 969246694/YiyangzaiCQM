...
JP = os.path.join(ROOT, 'lib', 'javaparser-core-3.26.4.jar')
...
''')
    ...
    # 服务端运行期依赖 H2（持久化）与 JavaParser（静态分析）
    SERVER_CP = ';'.join([os.path.join(ROOT, 'server', 'classes'), H2, JP])
    srv = subprocess.Popen(['java', '-cp', SERVER_CP, 'com.cqm.server.Server', '8080'],
